...
class TestHFTPServer(TestBase):
    ...
    def test_connect_and_quit(self):
        ...
        while got > 0 and time.clock() - start <= 0.5:
            ...
            self.assertEqual(
                r, [s], "Luego de la respuesta de quit, la "
                "conexión se mantuvo activa por más de 0.5 segundos")
            got = s.recv(1024)
        # Se desconectó?
        self.assert_(time > 0)
    ...
    def test_get_full_slice(self):
        ...
        self.assertEqual(c.status, constants.CODE_OK)
        self.assertEqual(
            open(self.output_file).read(), test_data,
    ...
    def test_partial_slices(self):
        ...
        self.assertEqual(c.status, constants.CODE_OK)
        self.assertEqual(
            open(self.output_file).read(), 'a' * 100,
            "El contenido del archivo no es el correcto")
        c.get_slice(self.output_file, 100, 200)
        self.assertEqual(c.status, constants.CODE_OK)
        self.assertEqual(
            open(self.output_file).read(),
...
class TestHFTPErrors(TestBase):
    ...
    def test_bad_argument_badfilename(self):
        c = self.new_client()
        c.send('get_metadata %s%s' % (('../' * 100), 'etc/passwd'))
        status, message = c.read_response_line(TIMEOUT)
        self.assertEqual(
            status, constants.INVALID_ARGUMENTS,
            "El servidor no contestó 201 ante un nombre de archivo "
            "invalido (status=%d)" % status)

        c.send('get_metadata bar@foo.com#imbox+spam?foo=bar&none<>')
        status, message = c.read_response_line(TIMEOUT)
        self.assertEqual(
            status, constants.INVALID_ARGUMENTS,
            "El servidor no contestó 201 ante un nombre de archivo "
